...
@st.cache_data
def load_data():    
    freq_lingua = pd.read_sql('SELECT tp_lingua, COUNT(*) AS frequencia FROM public.ed_enem_2024_resultados_amos_per GROUP BY tp_lingua ORDER BY frequencia DESC', engine)
    return freq_lingua

@st.cache_data
def load_uf():
    df_top5_uf = pd.read_sql('SELECT nome_uf_prova, COUNT(*) AS frequencia FROM public.ed_enem_2024_resultados_amos_per GROUP BY nome_uf_prova ORDER BY frequencia DESC LIMIT 5', engine)
    df_bottom5_uf = pd.read_sql('SELECT nome_uf_prova, COUNT(*) AS frequencia FROM public.ed_enem_2024_resultados_amos_per GROUP BY nome_uf_prova ORDER BY frequencia ASC LIMIT 5', engine)
    desc_uf = pd.read_sql('SELECT nome_uf_prova, COUNT(*) AS frequencia FROM public.ed_enem_2024_resultados_amos_per GROUP BY nome_uf_prova ORDER BY frequencia DESC', engine)
    return df_top5_uf , df_bottom5_uf, desc_uf

@st.cache_data
def load_matematica():
    nota_mt = pd.read_sql('SELECT nota_mt_matematica FROM public.ed_enem_2024_resultados_amos_per', engine)
    desc_mt = pd.read_sql('''SELECT
    COUNT(*) AS count,
    AVG(nota_mt_matematica) AS mean,
    STDDEV_SAMP(nota_mt_matematica) AS std,
    ...
    FROM public.ed_enem_2024_resultados_amos_per''', engine)
    return nota_mt, desc_mt

@st.cache_data
def load_linguagens():
    nota_lc = pd.read_sql('SELECT nota_lc_linguagens_e_codigos FROM public.ed_enem_2024_resultados_amos_per', engine)
    desc_lc = pd.read_sql('''SELECT
    COUNT(*) AS count,
    AVG(nota_lc_linguagens_e_codigos) AS mean,
    STDDEV_SAMP(nota_lc_linguagens_e_codigos) AS std,
...
# Top 5 estados com maior frequência
    fig1 = px.bar(df_top5, x='nome_uf_prova', y='frequencia',title='Estados com maior frequência',color_discrete_sequence=['lightgreen'])
    ...
    fig2 = px.bar(df_bottom5, x='nome_uf_prova', y='frequencia',title='Estados com menor frequência',color_discrete_sequence=['steelblue'])
...
with tab3:
    ...
    fig4.update_traces(
        marker=dict(
            color='#4F8EF7',
            line=dict(color='#1A5FC8', width=1.2)
        )
    )
